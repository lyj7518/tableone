...
<u>범주형 변수</u>는 개수(백분율)로 요약된다. <u>연속형 변수</u>는 normal인 경우 mean(sd)로 나타나고, nonnormal인 경우 median(IQR)로 요약된다.
...
앞에서 만든 table1에서 연속형 변수와 범주형 변수를 구분하여 출력할 수 있다. **"$"**를 사용하여 연속형 변수는 ConTable, 범주형 변수는 CaTable을 쓰면 해당 변수를 따로 출력할 수 있다.
...
```{r, echo=T}
print(t2,
      exact=c("Q_PHX_DX_STK", "Q_PHX_DX_HTDZ", "Q_PHX_DX_DLD"))
```
