...
```{r, echo=T}
# 범주형 변수 지정
myVars <- c("HGHT", "WGHT", "WSTC", "BMI", "HDL", "LDL", "Q_PHX_DX_STK", "Q_PHX_DX_HTDZ", "Q_PHX_DX_HTN", "Q_PHX_DX_DM", "Q_PHX_DX_DLD", "Q_HBV_AG", "Q_SMK_YN", "TG", "SGPT")
...
이진 범주 변수는 두 번째 level의 개수와 백분율로 요약된다. 예를 들어 0과 1의 범주가 있을 때, "1"의 요약값만 출력된다. 3개 이상의 범주가 있을 때에는 모든 level이 요약되며, 백분율은 누락된 값을 제외한 후 계산된다.

범주형 변수로 설정한 컬럼의 요약값이 mean(sd)에서 n(percentage)로 바뀐 것을 볼 수 있다.

## Showing all levels for categorical variables 

범주형 변수의 모든 levle을 확인하려면 *Show all levels* 옵션을 사용한다. 범주형 변수의 기본 설정은 0과 1의 범주가 있을 때, 1만 나타나지만 ShowAllLevels = T 설정 시 두개의 level을 모두 출력한다. 3개 이상의 level을 가진 변수는 모든 level이 자동적으로 출력된다. 
...
비모수적 방법을 사용하는 변수에는 **nonnormal** 옵션을 설정한다. 

```{r, echo=T}
print(t1, nonnormal="SGPT")
```
...
연속형 변수의 경우, 기본적으로 one-way ANOVA test가 적용되며 nonnormal일 경우 옵션 설정을 통해 Kruskal–Wallis one-way ANOVA를 적용할 수 있다.
...
앞에서 만든 table1에서 연속형 변수와 범주형 변수를 구분하여 볼 수 있다. **"$"**를 사용하여 연속형 변수는 ConTable, 범주형 변수는 CaTable을 쓰면 해당 변수를 따로 출력할 수 있다.
...
- `print`
  - showAllLevels : 2범주인 변수에서 2범주를 다 보여줄 변수
